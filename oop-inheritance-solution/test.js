...
var myMotorcycle = new Motorcycle('Triumph', 'Bonneville', '2015', 'green', 4);
assert.strictEqual(myMotorcycle instanceof Motorcycle, true, 'myMotorcycle not instance of Motorcycle');
assert.strictEqual(myMotorcycle instanceof Car, true, 'myMotorcycle is not instance of Car');
assert.strictEqual(myMotorcycle.seats <= 2, true, 'Seats not right');
assert.strictEqual(myMotorcycle.wheelie(), false, 'Motorcycle is running');
myMotorcycle.start();
assert.strictEqual(myMotorcycle.wheelie(), true, 'Not doing a wheelie or running');
success('Motorcycle is good');

console.log('Testing Truck');
var myTruck = new Truck('Ford', 'F150', '1995', 'black', 2);
...
assert.strictEqual(myTruck.year, '1995', 'Constructor did not set year');
assert.strictEqual(myTruck.color, 'black', 'Constructor did not set color');
assert.strictEqual(myTruck.seats >= 3 && myTruck.seats <= 4, true, 'Constructor did not set seats');
assert.strictEqual(myTruck.capacity, 0, 'Capacity was set even when it was not passed in');
assert.strictEqual(myTruck.cargo, 0, 'Cargo was set by the constructor');
myTruck.capacity = 300;
assert.strictEqual(myTruck.load(200), true, 'Cargo could not be loaded');
assert.strictEqual(myTruck.load(150), false, 'Cargo is not limited by capacity');
assert.strictEqual(myTruck.unload(50), true, 'Cargo could not be unloaded');
assert.strictEqual(myTruck.unload(300), false, 'Cargo unload is not limited by capacity');
success('The Truck is fine!');
